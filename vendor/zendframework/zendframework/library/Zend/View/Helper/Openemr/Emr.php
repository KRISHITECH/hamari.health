...
class Emr extends AbstractHelper {
    ...
    public function endInsurance($insrow, $indvarray) {

        foreach ($indvarray as $key => $val) {
            $$key = $val;
        }
        ...
        if ($form_csvexport != '') {
            ...
            $row_num++;
            $string.="  <td class='detail'>" . $row_num . "</td>\n";            // show row number  6/9/2010  HB
            $string.="  <td class='detail'>" . $insrow['insname'] . "</td>\n";
            $string.="  <td class='detotal' align='right'>&nbsp;" .
    ...
    public function endPatient($ptrow, $indvarray) {
        ...
        if (isset($form_csvexport) && $form_csvexport != '') {
            $export_patient_count += 1;
            $export_dollars += $pt_balance;
        } else if (isset($form_patientledger) && $form_patientledger != '') {
            // add a Patient Total line showing total charges for this patient
            // Patient Ledger            6/8/2010   HB
            $pt_ledger_str .= "\n";
            $pt_ledger_str .= sprintf("%s %68.2f\n", "Patient Total:", $ptrow['charges']);
            $pt_ledger_str .= "\014";
        } else {

            if ($ptrow['count'] > 1) {
                $string .= "  <tr style='background-color:#E0ECFF'>\n";
                $string .= "  <td class='detail' colspan='$initial_colspan'>";
                $string .= "  &nbsp;</td>\n";
                if (isset($form_cb_facility_name) && $form_cb_facility_name == true)
                    $string .= "  <td class='detail'>&nbsp;</td>\n";

                if (isset($form_cb_provider) && $form_cb_provider == true)
                    $string .= "  <td class='detail'>&nbsp;</td>\n";

                $string .= "  <td class='detail'>&nbsp;</td>\n";
                $string .= "  <td class='detotal' colspan='$final_colspan - 3'>&nbsp;Total Patient Balance:</td>\n";
                if ($form_age_cols) {
                    for ($c = 0; $c < $form_age_cols; ++$c) {
                        $string .= "  <td class='detotal' align='right'>&nbsp;" .
                ...
                $string .= "  <td class='detail' colspan='11'>&nbsp;</td>\n";
        ...
        $grand_total_copay += $ptrow['copayactualvalue'];
    ...
    /**
     * public function getarrmaster
     * to find whether anything exist or not
     */
    public function getarrmaster() {
        $sql = "SELECT * FROM arr_master";
        $result = sqlStatement($sql);
